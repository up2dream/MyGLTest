...
package cn.wps.moffice.presentation.interaction.view.gl.util_tmp;
...
public class NinePatchTexture extends ResourceTexture {
	...
	@Override
	public void draw(final IGLCanvas canvas, final int x, final int y, final int w, final int h) {
		if (!isLoaded()) {
	...
	public void recycle() {
		super.recycle();
		final IGLCanvas canvas = mCanvasRef;
	...
	private NinePatchInstance findInstance(final IGLCanvas canvas, final int w, final int h) {
		int key = w;
		key = key << 16 | h;
...
class NinePatchInstance {
	...
	public void draw(final IGLCanvas canvas, final NinePatchTexture tex, final int x, final int y) {
	...
	public void recycle(final IGLCanvas canvas) {
		if (mBufferNames != null) {
	...
	private void prepareBuffers(final IGLCanvas canvas) {
