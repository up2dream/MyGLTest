...
import cn.wps.moffice.presentation.interaction.view.gl.bs.BSGLSurfaceView;
import cn.wps.moffice.presentation.interaction.view.gl.bs.RenderLayerPh_GL;
...
//	        mGLSurfaceView.setRenderer(mGLSurfaceView);
	...
	public boolean onMenuItemSelected(int featureId, MenuItem item) {
		RenderLayerPh_GL renderLayer = mGLSurfaceView.getRenderLayer1();
...
//			mRenderer.mouseZoom(scale, new Point(mGLSurfaceView.getWidth()/2, mGLSurfaceView.getHeight()/2));
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			if (event.getPointerCount() > 1) {  
		        ...
		        if (Math.abs(newDist - oldDist) > 1) {
					...
					RenderLayerPh_GL layer = mGLSurfaceView.getRenderLayer1();
		...
		case MotionEvent.ACTION_POINTER_DOWN:
			oldWidth = mGLSurfaceView.getRenderLayer1().getWidth();
			oldHeight = mGLSurfaceView.getRenderLayer1().getHeight();
			oldDist = spacing(event);//���㰴��ʱ�ľ���  
			mGLSurfaceView.getRenderLayer1().setContentsFrozen(true);
			break;
		case MotionEvent.ACTION_POINTER_UP:
			mGLSurfaceView.getRenderLayer1().setContentsFrozen(false);
			break;
		}
